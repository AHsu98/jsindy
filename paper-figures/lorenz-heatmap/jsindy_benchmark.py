...
def get_args():
    ...
    p.add_argument("--num-repeats", type=int, default=0, help="Number of repetitions")
    return p.parse_args()
args = get_args()

import jax
print(jax.devices())
jax.config.update('jax_default_device',jax.devices()[args.gpu])
    ...
    for i,rkey in enumerate(all_keys):
        for tend in tqdm(tEndL):
            for noise_ratio in epsL:
                ...
                exp_name = f"seed{args.seed}_run_{i}_noise_ratio_{float(jnp.around(noise_ratio,3)):.3f}_t_end_{float(jnp.around(tend,3)):.3f}.pkl"
