...
class AnnealedAlternatingActiveSetLMSolver():
    ...
    def run(self, model, params):
        ...
        beta_reg_vals= [
            self.beta_reg * (self.anneal_beta_mult**(num_steps - i-1)) for i in range(num_steps)
                      ]
        ...
        for data_weight,colloc_weight,beta_reg in parameter_sequence:
            residual_function = partial(resid_func,data_weight = data_weight,colloc_weight = colloc_weight)
            ...
            if self.solver_settings.show_progress:
                print(f"Solving for data_weight = {data_weight}, colloc_weight = {colloc_weight} beta_reg = {beta_reg}")
            z_theta, lm_opt_results = CholeskyLM(
                z_theta_init, 
                lm_prob,
                self.beta_reg,
