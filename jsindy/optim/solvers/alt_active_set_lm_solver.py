...
def AlternatingActiveSolve(
    ...
    resfunc = jax.jit(lambda z,theta:residual_objective.F_split(z,theta.reshape(theta_shape)))
    ...
    def loss(z,theta):
        return (
            (1/2)*jnp.sum(resfunc(z,theta)**2) + 
            (beta/2)*z.T@residual_objective.state_param_regmat@z
        )
    ...
    def update_params(z,theta,prox_reg_init = 1.):
